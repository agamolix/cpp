...
ShrubberyCreationForm::ShrubberyCreationForm(void) : AForm("Shrubbery", 145, 137) {
	...
	std::cout << "Shrubbery created with name " << this->getName() << ", target = no target, signed = false and grades (145, 137)" << std::endl;
}

ShrubberyCreationForm::ShrubberyCreationForm(std::string t) : AForm("Shruberry", 145, 137) {
	this->setTarget(t);
	std::cout << "Shrubbery created with name " << this->getName() << ", target = " << t << ", signed = false and grades (" << this->getSignGrade() << " " << this->getExecGrade() << ")" << std::endl;
}

ShrubberyCreationForm::~ShrubberyCreationForm(void) {
	std::cout << "Shrubbery " << this->getName() << " destroyed" << std::endl;
}
...
void ShrubberyCreationForm::beSigned(Bureaucrat & b) {
	...
	this->setSigned(true);
	std::cout << "Shrubbery level " << this->getSignGrade() << " signed by Bureaucrat " << b.getName() << " with level " << b.getGrade() << std::endl;
}
