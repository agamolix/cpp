...
RobotomyRequestForm::RobotomyRequestForm(void) : AForm("Robotomy", 72, 45) { 
	...
	std::cout << "Robotomy created with name " << this->getName() << ", target = no target, signed = false and grades (72, 45)" << std::endl;
}

RobotomyRequestForm::RobotomyRequestForm(std::string t) : AForm("Robotomy", 72, 45) {
	this->setTarget(t);
	std::cout << "Robotomy created with name " << this->getName() << ", target = " << t << ", signed = false and grades (" << this->getSignGrade() << " " << this->getExecGrade() << ")" << std::endl;
}

RobotomyRequestForm::~RobotomyRequestForm(void) {
	std::cout << "Robotomy " << this->getName() << " destroyed" << std::endl;
}
...
void RobotomyRequestForm::beSigned(Bureaucrat & b) {
	...
	this->setSigned(true);
	std::cout << "Robotomy level " << this->getSignGrade() << " signed by Bureaucrat " << b.getName() << " with level " << b.getGrade() << std::endl;
}
