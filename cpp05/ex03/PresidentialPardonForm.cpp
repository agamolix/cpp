...
PresidentialPardonForm::PresidentialPardonForm(void) : AForm("Presidential pardon", 25, 5) { 
	...
	if (verbose)
		std::cout << "[PPF] created with name " << this->getName() << ", target = no target, signed = false and grades (72, 45)" << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(std::string t) : AForm("Presidential pardon", 25, 5) {
	this->setTarget(t);
	if (verbose)
		std::cout << "[PPF] created with name " << this->getName() << ", target = " << t << ", signed = false and grades (" << this->getSignGrade() << " " << this->getExecGrade() << ")" << std::endl;
}

PresidentialPardonForm::~PresidentialPardonForm(void) {
	if (verbose)
		std::cout << "[PPF] " << this->getName() << " destroyed" << std::endl;
}
...
void PresidentialPardonForm::beSigned(Bureaucrat & b) {
	...
	this->setSigned(true);
	std::cout << "[PPF] level " << this->getSignGrade() << " signed by Bureaucrat " << b.getName() << " with level " << b.getGrade() << std::endl;
}
