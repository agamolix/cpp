...
{
    os << "[AForm] " << instance.getName() << ", signed: " << std::boolalpha << instance.getSigned() << ", Grades " << instance.getSignGrade() << " " << instance.getExecGrade();
    return os;
}
...
void AForm::execute(Bureaucrat const & executor) const {
	...
	if (executor.getGrade() > this->getExecGrade())
		throw GradeTooLowException();
	this->exec();
	std::cout << "[AForm] " << this->_name << " was executed properly" << std::endl;
} 

bool AForm::verbose = true;
